...
describe("FiscalCode#toFiscalCode", () => {
  it("should returns a defined option for valid fiscal code", () => {
  ...
  it("should returns a empty option for malformed fiscal code", () => {
    expect(toFiscalCode("abc")).toEqual({});
  });
});

describe("FiscalCode#isFiscalCode", () => {
  it("should returns true if fiscal code is well formed", () => {
    const fiscalCode: FiscalCode = toPatternString(
      "AAABBB01C01A000A",
      "^[A-Z]{6}[0-9LMNPQRSTUV]{2}[ABCDEHLMPRST][0-9LMNPQRSTUV]{2}[A-Z][0-9LMNPQRSTUV]{3}[A-Z]$"
    ).get;
  ...
  it("should returns false if fiscal code is malformed", () => {
